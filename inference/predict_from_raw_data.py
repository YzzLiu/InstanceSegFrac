...
class InstanceSegFracPredictor(object):
    def __init__(self,
        ...
        self.predict_which = predict_which
        if device.type == 'cuda':
            pass
        if device.type != 'cuda':
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        if use_folds is None:
            use_folds = InstanceSegFracPredictor.auto_detect_available_folds(model_training_output_dir, checkpoint_name)
        ...
        trainer_class = recursive_find_python_class(join(nnunetv2.__path__[0], "training", "Trainer"),
                                                    trainer_name, 'Trainer')
        network = trainer_class.build_network_architecture(self, plans_manager, dataset_json, configuration_manager,
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if 'SAMed' in self.trainer_name:
            ...
            if self.predict_which == 1:
                prediction, prediction_2,_ = self.network(x)
            elif self.predict_which == 2:
                prediction_1, prediction,_ = self.network(x)
            elif self.predict_which == None:
        ...
        if mirror_axes is not None:
            ...
            # print("axes_combinations = ",axes_combinations)
            for axes in axes_combinations:
                if 'SAMed' in self.trainer_name:
                    prediction += torch.flip(self.network(torch.flip(x, (*axes,)),True, self.configuration_manager.patch_size[-1])['masks'], (*axes,))
                else:
                    if self.predict_which == 1 or self.predict_which == 2:
                        prediction_flip_1, prediction_filp_2,_ = self.network(torch.flip(x, (*axes,)))
                        if self.predict_which == 1:
                            prediction += torch.flip(prediction_flip_1, (*axes,))
                        elif self.predict_which == 2:
                            prediction += torch.flip(prediction_filp_2, (*axes,))
                    elif self.predict_which == None:
                        prediction_flip = self.network(torch.flip(x, (*axes,)))
...
if __name__ == '__main__':
    ...
    predictor = InstanceSegFracPredictor(
        tile_step_size=1,
        use_gaussian=True,
        use_mirroring=True,
    ...
    predictor.initialize_from_trained_model_folder(
        join(current_project, 'Model',dataset_name,model_name),
        use_folds=(fold_name, ),
        checkpoint_name='YourModelName.pth'
    )
    
    predictor.predict_from_files(join(current_project,'Data', dataset_name, 'imagesVal'),
                                 join(current_project, 'Result',dataset_name,model_name,'imagePr_' + fold_name,'InstanceResults'),
                                 save_probabilities=False, overwrite=True,
                                 num_processes_preprocessing=20, num_processes_segmentation_export=1,
